...
def generator(x, isTrain=True, reuse=False):
    with tf.variable_scope('generator', reuse=reuse):
        ...
        # output layer
        conv5 = tf.layers.conv2d_transpose(lrelu4, num_channels, [4, 4], strides=(2, 2), padding='same')
        o = tf.nn.tanh(conv5)
...
def show_result(num_epoch, show = False, save = False, path = 'result.png'):
    ...
    for k in range(size_figure_grid*size_figure_grid):
        ...
        ax[i, j].cla()
        ax[i, j].imshow(np.reshape(test_images[k], (img_size, img_size,num_channels)))
...
for epoch in range(train_epoch):
    ...
    for iter in range(100):#mnist.train.num_examples // batch_size
        # update discriminator
        #x_ = train_set[iter*batch_size:(iter+1)*batch_size]
        x_ = [np.reshape(b, [img_size, img_size,num_channels]) for b in data.next_batch(batch_size=batch_size)[0]]
